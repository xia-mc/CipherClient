...
public class Projectiles extends Module {
    ...
    public Projectiles() {
        super("projectiles", "Allows you to see all projectiles", Category.RENDER);
    }
    ...
    private void onWorldRender(RenderWorldLastEvent event) {
        ...
        while (!hasLanded) {
            ...
            MovingObjectPosition possibleLandingStrip = mc.theWorld.func_147447_a(present, future, false, true, true);
            ...
            List<Entity> entities = (List<Entity>) this.getEntitiesWithinAABB(arrowBox.addCoord(motionX, motionY, motionZ).expand(1.0, 1.0, 1.0));
        ...
        for (int x = chunkMinX; x < chunkMaxX; ++x) {
            for (int y = chunkMinY; y < chunkMaxY; ++y) {
                for (int z = chunkMinZ; z < chunkMaxZ; ++z) {
                    Block block = Block.getBlockById(mc.theWorld.getBlockMetadata(x, y, z));
                    if (block == null || block.getMaterial() != material || chunkMaxY < ((y + 1) - BlockLiquid.getLiquidHeightPercent(mc.theWorld.getBlockMetadata(x, y, z)))) {
                        continue;
                    }
                    isWithin = true;
                }
