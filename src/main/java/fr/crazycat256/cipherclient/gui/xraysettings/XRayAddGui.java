...
public class XRayAddGui extends GuiScreen {
        ...
        switch (par1GuiButton.id) {
            case 0:
                if (this.selectedIndex != -1) {
                    Modules.get().get(XRay.class).blocks.remove(this.selectedIndex);
                }
                Modules.get().get(XRay.class).blocks.add(new XRayBlock((int) (this.colorR.percent * 255.0f), (int) (this.colorG.percent * 255.0f), (int) (this.colorB.percent * 255.0f), (int) (this.colorA.percent * 255.0f), this.bmeta ? this.meta : -1, this.id, this.enabled));
                Modules.get().save();
                mc.displayGuiScreen(new XRayGui());
                break;
            case 1:
                mc.displayGuiScreen(new XRayGui());
                break;
    ...
    public void initGui() {
        ...
        GuiButton cancel = new GuiButton(1, this.width / 2 + 42, this.height - 22, 40, 20, "Cancel");
        this.buttonList.add(cancel);
        this.colorR = new XRayGuiSlider(2, this.width - 160, this.height / 10 * 5, "Red-Value", this.r / 255.0f);
        this.buttonList.add(this.colorR);
        this.colorG = new XRayGuiSlider(3, this.width - 160, this.height / 10 * 6, "Green-Value", this.g / 255.0f);
        this.buttonList.add(this.colorG);
        this.colorB = new XRayGuiSlider(4, this.width - 160, this.height / 10 * 7, "Blue-Value", this.b / 255.0f);
        this.buttonList.add(this.colorB);
        this.colorA = new XRayGuiSlider(5, this.width - 160, this.height / 10 * 8, "Alpha-Value", this.a / 255.0f);
    ...
    public void drawScreen(int x, int y, float par3) {
        ...
        for (si = 0; si < var13.size(); ++si) {
            ...
            try {
                ...
                XRayAddGui.drawRect((10 + si % var14 * 20), (60 + si / var14 * 20), (10 + si % var14 * 20 + 16), (60 + si / var14 * 20 + 16), -2130706433);
        ...
        RenderHelper.enableGUIStandardItemLighting();
        XRayAddGui.drawRect((this.width / 3 * 2), (this.height / 6), (this.width - 30), (this.height / 6 * 2), ((((int) (this.colorA.percent * 255.0f) << 8 | (int) (this.colorR.percent * 255.0f)) << 8 | (int) (this.colorG.percent * 255.0f)) << 8 | (int) (this.colorB.percent * 255.0f)));
        GL11.glDisable(2929);
    ...
    private ArrayList<?> getItemStackList() {
        ...
        for (String block : this.blocks) {
            Block b2 = (Block) Block.blockRegistry.getObject(block);
            b2.getSubBlocks(new ItemStack(b2).getItem(), null, blockstodraw);
        }
        return blockstodraw;
    }
    ...
    private Stringint getClickedBlock(int x, int y) {
        ...
        for (i = 0; i < this.blocks.size(); ++i) {
            b2 = (Block) Block.blockRegistry.getObject(this.blocks.get(i));
            b2.getSubBlocks(new ItemStack(b2).getItem(), null, z);
        }
        for (i = 0; i < this.blocks.size(); ++i) {
            b2 = (Block) Block.blockRegistry.getObject(this.blocks.get(i));
            ArrayList<?> blockstodraw = new ArrayList<>();
            ...
            for (int j = 0; j < blockstodraw.size(); ++j) {
                int var14;
                if ((index + j - this.sliderpos * 9) / 9 > 7 || index + j - this.sliderpos * 9 < 0) {
                    continue;
                }
                int c = (index + j) % 9;
                ...
                return new Stringint(this.blocks.get(i), var14);
            }
